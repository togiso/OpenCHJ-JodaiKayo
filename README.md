...
このXMLでは、文書全体を `<doc>` 要素で囲み、OCX mini の名前空間 `https://openchj.github.io/ns/ocx` をデフォルト名前空間として指定しています。各歌は和歌一首を表す単位として `<lg type="waka">` で記述し、元データの識別子は `id` 属性として保持しました。
